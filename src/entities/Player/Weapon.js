...
export default class Weapon extends Component{
    ...
    SetVisible(visible) {
        if (this.weaponModel) { // Vérifier si weaponModel est bien défini
          this.weaponModel.visible = visible; // Modifier la visibilité du modèle de l'arme
        } else {
        //   console.error(`weaponModel is undefined for weapon: ${this.name}`);
        }
      }
    ...
    Update(t) {
        if (!this.isVisible) return; // Si l'arme n'est pas visible, ne pas la mettre à jour
        // Reste de la logique de tir, rechargement, etc.
      }
    ...
    Shoot(t){
        ...
        if(this.shootTimer <= 0.0 ){
            ...
            this.Raycast();
            this.Broadcast({topic: 'raygun_shot'});
