...
import scar from './assets/guns/scar/scar.glb';
import muzzleFlash from './assets/muzzle_flash.glb'
//Shot sound
import ak47Shot from './assets/sounds/ak47_shot.wav'
// import ak47Shot from './assets/sounds/raygun_shot.mp3'


import menumusic from './assets/sounds/menusong.mp3'
...
class FPSGameApp{
  ...
  async LoadAssets(){
    ...
    promises.push(this.AddAsset(scar, gltfLoader, "scar"));
    // promises.push(this.AddAsset(raygunShot, audioLoader, "raygunShot"));
    //Ammo box
    ...
    this.assets['scar'].scene.animations = this.assets['scar'].animations;
  ...
  EntitySetup(){
    ...
    playerEntity.AddComponent(new PlayerControls(this.camera, this.scene));
     // Ajouter les deux armes au joueur avant de les utiliser dans PlayerControls
    const ak47Weapon = new Weapon(this.camera, this.assets['ak47'].scene, this.assets['muzzleFlash'], this.physicsWorld, this.assets['ak47Shot'], this.listener);
    ak47Weapon.name = "WeaponAK47"; // Donnez un nom explicite à l'arme
    playerEntity.AddComponent(ak47Weapon);

    const scarWeapon = new Weapon(this.camera, this.assets['scar'].scene, this.assets['muzzleFlash'], this.physicsWorld, this.assets['ak47Shot'], this.listener);
    scarWeapon.name = "WeaponScar"; // Donnez un nom explicite à l'arme
    playerEntity.AddComponent(scarWeapon);
   ...
   const npcLocations = [
      [10.8, 0.0, 22.0], [11.8, 0.0, 22.5], [12.8, 0.0, 23.0], [13.8, 0.0, 23.5],
      [14.8, 0.0, 24.0], [15.8, 0.0, 24.5], [16.8, 0.0, 25.0], [17.8, 0.0, 25.5],
      [18.8, 0.0, 26.0], [19.8, 0.0, 26.5]
    ]; 
